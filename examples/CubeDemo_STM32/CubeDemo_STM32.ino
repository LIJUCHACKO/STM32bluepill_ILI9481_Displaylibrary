...
void cube(float *px, float *py, float *pz, float *p2x, float *p2y, int *r, uint16 *x, uint16 *y, uint16 *color) {
  ...
  for (int i = 0; i < 3; i++) {
    tft.Draw_Lin(p2x[i], p2y[i], p2x[i + 1], p2y[i + 1], BACK_COLOR);
    tft.Draw_Lin(p2x[i + 4], p2y[i + 4], p2x[i + 5], p2y[i + 5], BACK_COLOR);
    tft.Draw_Lin(p2x[i], p2y[i], p2x[i + 4], p2y[i + 4], BACK_COLOR);
  }
  tft.Draw_Lin(p2x[3], p2y[3], p2x[0], p2y[0], BACK_COLOR);
  tft.Draw_Lin(p2x[7], p2y[7], p2x[4], p2y[4], BACK_COLOR);
  tft.Draw_Lin(p2x[3], p2y[3], p2x[7], p2y[7], BACK_COLOR);
  ...
  for (int i = 0; i < 3; i++) {
    tft.Draw_Lin(p2x[i], p2y[i], p2x[i + 1], p2y[i + 1], *color);
    tft.Draw_Lin(p2x[i + 4], p2y[i + 4], p2x[i + 5], p2y[i + 5], *color);
    tft.Draw_Lin(p2x[i], p2y[i], p2x[i + 4], p2y[i + 4], *color);
  }
  tft.Draw_Lin(p2x[3], p2y[3], p2x[0], p2y[0], *color);
  tft.Draw_Lin(p2x[7], p2y[7], p2x[4], p2y[4], *color);
  tft.Draw_Lin(p2x[3], p2y[3], p2x[7], p2y[7], *color);
}
...
//  uint32_t ID = tft.readID();
 ...
 // tft.begin(ID);
  ...
  uint32_t width = tft.Get_Width();
  Serial.print("Width: "); Serial.println(width);
  uint32_t height = tft.Get_Height();
  Serial.print("Height: "); Serial.println(height);
  
  tft.LCD_Clear(BACK_COLOR);
  cube1_x = ((tft.Get_Width()) / 4);
  cube1_y = ((tft.Get_Height()) / 4);
  cube2_x = ((tft.Get_Width()) / 2);
  cube2_y = ((tft.Get_Height()) / 2);
  cube1_color = BLACK;
  cube2_color = RED;
